...
export default function Home() {
  ...
  // nouvel état : affichage du Mood Dashboard (modale)
  ...
  return (
    <>
      <main className="min-h-screen bg-gradient-to-b from-black via-slate-950 to-slate-900 text-white flex flex-col items-center px-4 py-6">
        ...
        {/* BOUTON "Voir mon suivi de mood" (centré, visible seulement si connecté) */}
        {user && (
          <div className="w-full max-w-4xl flex justify-center mb-6">
            <button
              onClick={() => setShowMoodDashboard(true)}
              className="rounded-2xl bg-gradient-to-r from-rose-500 via-orange-400 to-yellow-400 text-black px-6 py-3 font-semibold shadow-2xl hover:scale-105 transition"
            >
        ...
        <footer className="mt-2 text-[10px] text-neutral-500 text-center max-w-4xl pb-4">
          Prototype v1 · Les données de mood sont stockées localement sur ton appareil
          (connexion à la base Supabase dans la suite).
        </footer>
      </main>

      {/* MODALE MoodDashboard (SSR false) */}
      {showMoodDashboard && (
          ...
          <div className="w-full max-w-5xl mx-auto">
            <div className="flex justify-end mb-3">
              <button
                ...
                className="text-white/90 bg-black/40 px-3 py-1 rounded-full"
              >
                Fermer ✕
              </button>
            </div>
            {/* MoodDashboard: composant dynamique (placer app/components/MoodDashboard.tsx) */}
            <MoodDashboard initial={moodLogs} enableSupabaseSync={true} />
          </div>
        </div>
      )}
    </>
