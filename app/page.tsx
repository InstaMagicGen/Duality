...
export default function Home() {
  ...
  return (
    ...
    >
      {/* --- Background Ambient Glows (Effets cosmiques) --- */}
      ...
      <div className="w-full max-w-6xl relative z-10 flex flex-col">
        ...
        {/* Sélecteur de langue discret (Optionnel mais demandé dans les specs) */}
        <div className="absolute top-0 right-0 -mt-4 flex gap-2">
            {(['fr', 'en', 'ar'] as Lang[]).map((l) => (
                <button 
                    key={l} 
                    onClick={() => setLang(l)}
                    className={`text-xs uppercase px-2 py-1 rounded ${lang === l ? 'bg-white/20 text-white' : 'text-gray-600'}`}
                >
                    {l}
                </button>
            ))}
        ...
        <section className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-20">
          ...
          {/* DUALITY CARD (Yellow) */}
          <div className="glow-card-yellow rounded-3xl p-8 flex flex-col justify-between min-h-[300px] relative group">
            <div>
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4 mt-auto">
              <span className="text-[10px] font-bold tracking-[0.2em] text-yellow-600/80 uppercase">
                {t.duality.tags}
              </span>
          ...
          {/* SOULSET CARD (Blue) */}
          <div className="glow-card-blue rounded-3xl p-8 flex flex-col justify-between min-h-[300px] relative group">
            <div>
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4 mt-auto">
              <span className="text-[10px] font-bold tracking-[0.2em] text-cyan-700/80 uppercase">
                {t.soulset.tags}
              </span>
