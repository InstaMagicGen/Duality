...
export default function Home() {
  ...
  useEffect(() => {
    ...
    async function initSession() {
      try {
        const { data: { session, user: supUser }, error } = await supabase.auth.getSession();
        if (error) console.warn("getSession error", error);
        else if (mounted && (session?.user || supUser)) {
          const u = session?.user ?? supUser;
        ...
        <section className="w-full max-w-4xl mb-6">
          {user ? (
            ...
            <div className="rounded-3xl border border-neutral-700/80 bg-black/80 p-6 md:p-7 shadow-[0_0_40px_rgba(15,23,42,0.7)] flex flex-col md:flex-row items-center justify-between gap-4">
              ...
              <Link href="/auth?mode=login" className="mt-3 md:mt-0 rounded-full bg-gradient-to-r from-yellow-400 via-amber-300 to-yellow-500 text-black px-6 py-2 text-sm font-semibold hover:brightness-110 transition shadow-[0_0_25px_rgba(250,204,21,0.6)]">{t.moodLockedButton}</Link>
            </div>
          )}
        </section>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4">
          <div className="w-full max-w-5xl mx-auto">
            <div className="flex justify-end mb-3">
              <button onClick={() => setShowMoodDashboard(false)} className="text-white/90 bg-black/40 px-3 py-1 rounded-full">Fermer ✕</button>
            </div>
